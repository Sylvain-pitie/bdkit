...
# Définissez le nom de votre fichier
    ...
    nom_fichier = "./band/PBAND_"+atm4+".dat"
    ...
    raw2 = donnees["Colonne2"].tolist()
    donnees["Colonne3"] = donnees["Colonne3"] * 10
    raws4 = donnees["Colonne3"].tolist()
    donnees["Somme"] = donnees["Colonne4"] + donnees["Colonne5"] + donnees["Colonne6"]
    donnees["Somme"] = donnees["Somme"] * 10
    rawp4 = donnees["Somme"].tolist()
    donnees["Sommed"] = donnees["Colonne7"] + donnees["Colonne8"] + donnees["Colonne9"] + donnees["Colonne10"] + donnees["Colonne11"]
    donnees["Sommed"] = donnees["Sommed"] * 10
    rawd4 = donnees["Sommed"].tolist()
    donnees["Sommet"] = donnees["Colonne3"] + donnees["Somme"] + donnees["Sommed"]
    rawt4 = donnees["Sommet"].tolist()
...
# Convertissez les données en listes
    ...
    ndos["Somme"] = ndos["Colonne6"] + ndos["Colonne7"] + ndos["Colonne8"] + ndos["Colonne9"] + ndos["Colonne10"]
    dosd3tmp = ndos["Somme"].tolist()
    ndos["Sommet"] = ndos["Colonne2"] + ndos["Somme"] + ndos["Sommed"]
    dost3tmp = ndos["Sommet"].tolist()
#########Lecture de la structure de bandes projeté sur N###########################
# Définissez le nom de votre fichier
    ndosfile = "./dos/PDOS_"+atm4+".dat"
...
# Convertissez les données en listes
    dos1tmp = ndos["Colonne1"].tolist()
    doss4tmp = ndos["Colonne2"].tolist()
    ndos["Somme"] = ndos["Colonne3"] + ndos["Colonne4"] + ndos["Colonne5"]
    dosp4tmp = ndos["Somme"].tolist()
    ndos["Somme"] = ndos["Colonne6"] + ndos["Colonne7"] + ndos["Colonne8"] + ndos["Colonne9"] + ndos["Colonne10"]
    dosd4tmp = ndos["Somme"].tolist()
    ndos["Sommet"] = ndos["Colonne2"] + ndos["Somme"] + ndos["Sommed"]
    dost4tmp = ndos["Sommet"].tolist()
...
################################################interpolate############################################
    ...
    doss4tmp = np.array(doss4tmp)
    dosp4tmp = np.array(dosp4tmp)
    dosd4tmp = np.array(dosd4tmp)
    dost4tmp = np.array(dost4tmp)
    ...
    interpolation4s = interp1d(dos1tmp, doss4tmp, kind='cubic')
    interpolation4p = interp1d(dos1tmp, dosp4tmp, kind='cubic')
    interpolation4d = interp1d(dos1tmp, dosd4tmp, kind='cubic')
    interpolation4t = interp1d(dos1tmp, dost4tmp, kind='cubic')
    ...
    doss4 = interpolation4s(dos1)
    dosp4 = interpolation4p(dos1)
    dosd4 = interpolation4d(dos1)
    dost4 = interpolation4t(dos1)
...
#######################################################################################################
    ...
    for f in range(len(typeorb4)):
        if str(typeorb4[f]) == "s":
            plt.scatter(raw1,raw2, s=raws4, facecolors='none', alpha=1.0, color=color4[f], zorder=2, label=atm4+'-s')
        elif str(typeorb4[f]) == "p":
            plt.scatter(raw1,raw2, s=rawp4, facecolors='none', alpha=1.0, color=color4[f], zorder=2, label=atm4+'-p')
        elif str(typeorb4[f]) == "d":
            plt.scatter(raw1,raw2, s=rawd4, facecolors='none', alpha=1.0, color=color4[f], zorder=2, label=atm4+'-d')
        elif str(typeorb4[f]) == "t":
            plt.scatter(raw1,raw2, s=rawt4, facecolors='none', alpha=1.0, color=color4[f], zorder=2, label=atm4)
...
#    plt.legend(loc="upper right")
    ...
    for f in range(len(typeorb4)):
        if str(typeorb4[f]) == "s":
            plt.plot(doss4,dos1, color=color4[f], zorder=2, label=atm4+'-s')
        elif str(typeorb4[f]) == "p":
            plt.plot(dosp4,dos1, color=color4[f], zorder=2, label=atm4+'-p')
        elif str(typeorb4[f]) == "d":
            plt.plot(dosd4,dos1, color=color4[f], zorder=2, label=atm4+'-d')
        elif str(typeorb4[f]) == "t":
            plt.plot(dost4,dos1, color=color4[f], zorder=2, label=atm4)
